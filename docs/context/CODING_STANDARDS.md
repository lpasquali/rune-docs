...
- **Testing**:
  ...
  - Unit tests should be safe/offline (mock boundaries). When integrating HTTP clients (like `httpx`), always use mocking libraries (like `respx`) to simulate API responses and achieve 100% coverage on the new file.
